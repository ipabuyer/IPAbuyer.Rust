...
1. Rust：`cd src-tauri && cargo test`——core 并入的 119 项 + 应用层（配置序列化兼容、密钥轮换默认值、日志缓冲环形上限、消息序列化、DTO 映射等）共 141 项；修改 core 或命令层必须保证通过。
2. 前端：`pnpm test`（Vitest 5 + jsdom，`pnpm test:watch` 常驻）——覆盖 lib/ 纯函数（价格/状态策略、URL 拼装、cn）、stores（会话/搜索/队列/日志，mock Tauri invoke 与 event）、消息渲染 hook（{{0}} 插值与键名回退），以及组件冒烟测试（SettingsCard 结构、AppSidebar 导航与外链标识、日志窗口清空/关闭/标题）。新增 UI 文本逻辑或组件时应配套用例。
